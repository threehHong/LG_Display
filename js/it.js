...
function event_Bezel(){
  if (windowScroll > figBezelPos) {
    ...
    bezelImg.attr({src: `img/bezelessimg/OVERVIEW_DESIGN_PC_FHD_000${srcNum.substr(-2)}.jpg`,});
  }
}
...
function event_Sketch(){
  ...
  penTouch.each(function(idx, item){
    penTouch.eq(idx).css({
      ...
      "background-position-x" : `-${(penTouch.eq(idx).offset().left) - figSketchPos.left}px`,
      "background-position-y" : `-${(penTouch.eq(idx).offset().top) - figSketchPos.top}px`,
    })
  })
